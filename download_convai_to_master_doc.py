...
# ----------------- НАСТРОЙКИ -----------------
API_KEY       = "sk_…"  # ваш ключ
PAGE_SIZE     = 100
MIN_DURATION  = 60      # секundy
...
# ----------------- Вспомогалки -----------------
def load_last_run():
    if not os.path.exists(LAST_RUN_FILE): return 0
...
def format_call(detail, fallback_ts):
    ...
    lines = []
    prev_role = None
    for m in detail.get("transcript", []):
...
def main():
    ...
    for c in new_calls:
        ...
        if st_call > max_ts: max_ts = st_call

    # 3) Вставляем только текст
    requests_body = [{"insertText": {"location": {"index": 1}, "text": full_text}}]
    try:
