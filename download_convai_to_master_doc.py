...
# Сдвиг вашего часового пояса относительно UTC (например, +4)
...
# Не тянем прокси из окружения, чтобы не было ошибок кодировки
...
def format_call(detail, fallback_ts):
    # стартовое время звонка по UTC+OFFSET
...
def main():
    ...
    # 2) Скачиваем все звонки
    calls = fetch_all_calls()

    # 3) Фильтрация по дате и длительности
    sel = [
        c for c in calls
    ...
    # 5) Сортировка: последние звонки первыми
    new_calls.sort(key=lambda x: x["start_time_unix_secs"], reverse=True)

    # 6) Формируем full_text и max_ts
    full_text = ""
    max_ts = last_ts
    for c in new_calls:
    ...
    # Выполняем batchUpdate точно как локально
